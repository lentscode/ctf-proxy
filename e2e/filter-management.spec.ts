...
test("operator can manage a structured filter from its proxy", async ({
  page,
}) => {
  ...
  const proxyDirectory = page
    .getByRole("button", { name: "web-http" })
    .locator("..");
  await proxyDirectory
    .getByRole("link", { name: "Manage filters · 0" })
    .click();
  ...
  await page
    .getByRole("group", { name: "Filter block-admin" })
