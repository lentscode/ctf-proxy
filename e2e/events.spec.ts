...
test("operator sees a new control event from the live event stream", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("configuration update rejected", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText("control · control_configuration_rejected", { exact: true }),
  ).toBeVisible();
});
...
test("operator can retry a failed event-history query", async ({ page }) => {
        ...
        events: [
          {
            ...
            component: "control",
            kind: "recovered",
            message: "Recovered event history",
          },
        ],
      }),
    });
  ...
  await expect(
    page.getByText("Recovered event history", { exact: true }),
  ).toBeVisible();
});
...
test("operator ignores an invalid event-stream payload", async ({ page }) => {
  ...
  await expect(page.getByText("untrusted event", { exact: true })).toHaveCount(
    0,
  );
});

test("operator is signed out when a reconnected event stream loses authorization", async ({
  page,
}) => {
  let connections = 0;
  await page.route("**/api/v1/events/stream", async (route) => {
  ...
  await expect
    .poll(() =>
      page.evaluate(() => sessionStorage.getItem("ctf-proxy.auth-token")),
    )
    .toBeNull();
});

test("operator sees the event stream reconnect after it closes", async ({
  page,
}) => {
  let connections = 0;
  await page.route("**/api/v1/events/stream", async (route) => {
