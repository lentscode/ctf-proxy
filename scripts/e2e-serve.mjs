...
async function waitForControl() {
  ...
  while (Date.now() < deadline) {
    if (
      controlProcess.exitCode !== null ||
      controlProcess.signalCode !== null
    ) {
...
try {
  ...
  await writeFile(
    join(composeRoot, "demo", "compose.yaml"),
    'services:\n  web:\n    image: example\n    ports:\n      - "18080:80"\n',
    { mode: 0o600 },
  );
  await writeFile(
...
`,
  ...
  const frontendBuild = command(
    process.platform === "win32" ? "pnpm.cmd" : "pnpm",
    ["run", "build:frontend"],
  );
  if ((await waitForExit(frontendBuild)) !== 0)
    throw new Error("could not build dashboard for E2E tests");

  const build = command("go", [
    "build",
    "-tags",
    "production",
    "-o",
  ...
  if ((await waitForExit(build)) !== 0)
    throw new Error("could not build ctf-proxy for E2E tests");
