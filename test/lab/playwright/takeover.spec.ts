...
test("operator applies or restores the four-service real-world lab takeover", async ({
  page,
}) => {
  ...
  await expect(
    restoreAll.or(page.getByText("No managed deployments.")),
  ).toBeVisible();
  ...
  for (const service of [
    ...
    await expect(
      page.getByText(new RegExp(`${service} \\(compose\\.yaml\\)`)),
    ).toBeVisible();
  }
  for (const service of [
    "tcp-echo",
  ...
  await expect(page.getByLabel(/Protocol for http-template/)).toHaveValue(
    "http",
  );
