...
export function ScanAndConfigureModal({ discovery, onClose, onUnauthorized, open }: Props) {
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key !== 'Escape' || apply.isPending || restore.isPending) return
      if (applyDialogOpen) {
        setApplyDialogOpen(false)
        return
      }
  ...
  }, [apply.isPending, applyDialogOpen, closeScanModal, open, restore.isPending])
  ...
  if (!open) return null
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !apply.isPending && !restore.isPending) closeScanModal() }}>
    <section className="max-h-[calc(100svh-2rem)] w-full max-w-5xl overflow-y-auto rounded-lg border border-zinc-700 bg-zinc-950 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="scan-and-configure-heading">
      <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-zinc-700 bg-zinc-950 px-6 py-5">
        ...
        <button type="button" className="min-h-9 rounded-md border border-zinc-600 px-3 text-sm font-semibold text-zinc-100 hover:border-zinc-100 disabled:opacity-60" onClick={closeScanModal} disabled={apply.isPending || restore.isPending}>Close</button>
      </header>
    ...
    {applyDialogOpen && <div className="fixed inset-0 z-[60] grid place-items-center bg-black/70 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !apply.isPending) setApplyDialogOpen(false) }}>
      <section className="w-full max-w-md rounded-lg border border-zinc-700 bg-zinc-950 p-6 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="apply-configuration-heading" aria-describedby="apply-configuration-description">
        <h2 id="apply-configuration-heading" className="m-0 text-xl font-semibold text-zinc-100">Confirm</h2>
        <p id="apply-configuration-description" className="mb-6 text-sm text-zinc-300">This recreates affected services and starts proxies.</p>
        <div className="flex justify-end gap-3"><button type="button" className="min-h-9 rounded-md px-3 text-sm font-semibold text-zinc-300 hover:text-zinc-100 disabled:opacity-50" onClick={() => setApplyDialogOpen(false)} disabled={apply.isPending}>Cancel</button><button type="button" className="min-h-9 rounded-md border border-zinc-400 px-3 text-sm font-semibold text-zinc-100 hover:border-zinc-100 disabled:cursor-wait disabled:opacity-50" disabled={apply.isPending} onClick={() => void apply.mutateAsync()}>{apply.isPending ? 'Applying…' : 'Confirm'}</button></div>
      </section>
    </div>}
  </div>
}
