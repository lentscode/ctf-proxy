...
export function EventStream({ onUnauthorized }: EventStreamProps) {
  ...
  const events = useMemo(
    () => mergeEvents(liveEvents, history ?? []),
    [history, liveEvents],
  );
  ...
  return (
    <section
      className="relative flex min-w-0 flex-col overflow-hidden border-l border-zinc-700 max-lg:border-l-0"
    ...
    >
      <header className="flex min-h-15 items-center border-b border-zinc-700 px-5">
        ...
        <span className="ml-auto rounded-full bg-zinc-800 px-2 py-1 text-[11px] leading-none font-semibold text-zinc-300 capitalize">
          {status}
        </span>
      </header>
      <div
  ...
  return (
    <li className="border-b border-zinc-700 px-5 py-3.5">
      <div className="grid grid-cols-[auto_auto_minmax(0,1fr)] items-start gap-2">
        <time
          dateTime={event.time}
          className="pt-0.5 font-mono text-[11px] leading-tight whitespace-nowrap text-zinc-400"
        >
          {timestamp}
        </time>
        <span className="rounded-full bg-zinc-800 px-1.5 py-1 text-[11px] leading-none font-semibold text-zinc-200 uppercase">
          {event.level}
        </span>
        <p className="m-0 mt-0.5 break-words text-sm leading-snug text-zinc-100">
          {event.message}
        </p>
      </div>
      <p className="mt-1.5 ml-[93px] break-words font-mono text-[11px] leading-snug text-zinc-400">
        {metadata.join(" · ")}
      </p>
    </li>
  );
...
function EventSkeleton() {
  return (
    <div className="grid gap-3 p-5" aria-label="Loading events">
      {Array.from({ length: 5 }, (_, index) => (
        <span key={index} className="h-10 animate-pulse rounded bg-zinc-900" />
      ))}
    </div>
  );
}
