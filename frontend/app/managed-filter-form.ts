...
export const matchFieldSchema = z.enum([
  "tcp.body",
  "http.path",
  "http.header",
  "http.body",
...
export const managedFilterDraftSchema = z
  .object({
    ...
    for (const [index, condition] of draft.conditions.entries()) {
      if (!isFieldAvailable(condition.field, draft.protocol, draft.direction)) {
        context.addIssue({
          ...
          message:
            "This field is not available for the selected protocol and direction.",
        });
      }
...
export function createEmptyDraft(protocol: FilterProtocol): ManagedFilterDraft {
  return {
    ...
    conditions: [
      {
        field: availableFields(protocol, "request")[0],
        header: "",
        operator: "contains",
        value: "",
      },
...
export function parseManagedFilterYAML(source: string): ManagedFilterDraft {
  const document = yamlDocumentSchema.parse(
    parse(source, { schema: "failsafe" }),
  );
      ...
      filters: [
        {
          ...
          action: "reject",
          match: {
            all: valid.conditions.map(({ field, header, operator, value }) => ({
              field,
