...
import {
  availableFields,
  filterDirectionSchema,
  filterProtocolSchema,
  labelForField,
  managedFilterDraftSchema,
  matchOperatorSchema,
...
export function ManagedFilterForm({
  ...
  function updateCondition(
    index: number,
    update: Partial<ManagedFilterDraft["conditions"][number]>,
  ) {
    setDraft((current) => ({
      ...current,
      conditions: current.conditions.map((condition, currentIndex) =>
        currentIndex === index ? { ...condition, ...update } : condition,
      ),
    }));
    setValidationError(undefined);
  }
  ...
  function addCondition() {
    setDraft((current) => ({
      ...
      conditions: [
        ...current.conditions,
        {
          field: availableFields(current.protocol, current.direction)[0],
          header: "",
          operator: "contains",
          value: "",
        },
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!parsed.success) {
      const message =
        ...
        "Check the filter configuration and try again.";
  ...
  return (
    <form
      className="grid gap-5 border-t border-zinc-700 px-5 py-5"
      onSubmit={(event) => void submit(event)}
    >
      <ManagedFilterIntroduction
        isExisting={isExisting}
        name={initial.name}
        assignedProxies={assignedProxies}
      />
      <ManagedFilterSettings
        draft={draft}
        isExisting={isExisting}
        onNameChange={(name) => {
...
function ManagedFilterIntroduction({
  isExisting,
  name,
  assignedProxies,
}: {
    ...
    <>
      <div className="grid gap-1">
        <h3 className="m-0 text-sm font-semibold text-zinc-100">
          {isExisting ? `Edit ${name}` : "Add managed filter"}
        </h3>
        <p className="m-0 text-xs leading-relaxed text-zinc-400">
          This rule rejects traffic only when every condition below matches.
        </p>
      </div>
      {assignedProxies.length > 1 && (
        <p
          className="m-0 border-l-2 border-zinc-300 bg-zinc-900 px-3 py-2 text-xs leading-relaxed text-zinc-200"
          role="note"
        >
          This filter is shared by {assignedProxies.length} proxies. Saving
          updates and restarts every assigned proxy:{" "}
          {assignedProxies.join(", ")}.
        </p>
      )}
    </>
  );
...
function ManagedFilterSettings({
  draft,
  isExisting,
  onNameChange,
  onProtocolChange,
  ...
  onDirectionChange: (direction: FilterDirection) => void;
    ...
    <>
      <div className="grid grid-cols-3 gap-4 max-sm:grid-cols-1">
        <label className="grid gap-1.5 text-xs font-semibold text-zinc-400">
          Filter name
          <input
            className="h-10 w-full rounded-md border border-zinc-600 bg-zinc-950 px-2.5 font-mono text-xs text-zinc-100 outline-none transition focus:border-zinc-100 focus:ring-3 focus:ring-white/10 disabled:cursor-not-allowed disabled:opacity-60"
            value={draft.name}
        ...
        <label className="grid gap-1.5 text-xs font-semibold text-zinc-400">
          ...
          <select
            ...
            onChange={(event) =>
              onProtocolChange(filterProtocolSchema.parse(event.target.value))
            }
          >
        ...
        <label className="grid gap-1.5 text-xs font-semibold text-zinc-400">
          ...
          <select
            ...
            onChange={(event) =>
              onDirectionChange(filterDirectionSchema.parse(event.target.value))
            }
          >
...
function ManagedFilterConditions({
  draft,
  onUpdate,
  onRemove,
  onAdd,
  ...
  return (
    <fieldset className="grid gap-3 border-0 p-0">
      ...
      <button
        type="button"
        className="justify-self-start cursor-pointer bg-transparent p-0 text-xs font-semibold text-zinc-200 underline underline-offset-3"
        onClick={onAdd}
      >
        Add condition
      </button>
    ...
    <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto] items-end gap-3 border border-zinc-700 p-3 max-lg:grid-cols-2 max-sm:grid-cols-1">
      <label className="grid gap-1.5 text-xs font-semibold text-zinc-400">
        ...
        <select
          ...
          onChange={(event) => {
            ...
            onUpdate({
              field,
              header: field === "http.header" ? condition.header : "",
            });
          }}
        >
          {fields.map((field) => (
      ...
      <label className="grid gap-1.5 text-xs font-semibold text-zinc-400">
        ...
        <select
          ...
          onChange={(event) =>
            onUpdate({
              operator: matchOperatorSchema.parse(event.target.value),
            })
          }
        >
          {matchOperatorSchema.options.map((operator) => (
      ...
      <label className="grid gap-1.5 text-xs font-semibold text-zinc-400">
        Match value
        <input
          aria-label={`Condition ${index + 1} match value`}
      ...
      {condition.operator === "regex" && (
        <p className="col-span-full m-0 text-xs leading-relaxed text-zinc-400">
          Regular expressions use RE2 syntax, so look-arounds such as{" "}
          <code>(?!checker)</code> are not supported. Choose “Does not contain”
          to reject a header without <code>checker</code>.
        </p>
      )}
    </div>
  );
...
function normalizeDraft(draft: ManagedFilterDraft): ManagedFilterDraft {
  ...
  return {
    ...
    conditions: draft.conditions.map((condition) => {
      const field = fields.includes(condition.field)
        ? condition.field
        : fields[0];
      return {
        ...condition,
